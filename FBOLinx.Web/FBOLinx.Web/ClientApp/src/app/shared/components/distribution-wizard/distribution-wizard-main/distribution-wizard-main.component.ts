...
/** distribution-wizard-main component*/
export class DistributionWizardMainComponent implements OnInit {
    ...
    public isDistributionComplete: boolean = false;
    public firstFormGroup: FormGroup;
    public secondFormGroup: FormGroup;
    public thirdFormGroup: FormGroup;
    public fourthFormGroup: FormGroup;
    public fifthFormGroup: FormGroup;
    public isLoadingCustomers: boolean = true;
    public isLoadingPricingTemplates: boolean = true;
    public isLoadingEmailContent: boolean = true;
    public isLoadingPreview: boolean = true;
    ...
    /** distribution-wizard-main ctor */
    constructor(public dialogRef: MatDialogRef<DistributionWizardMainComponent>, @Inject(MAT_DIALOG_DATA) public data: DistributionDialogData,
    ...
    public pricingTemplateChanged() {
        this.data.pricingTemplate = this.firstFormGroup.get('pricingTemplate').value;
        this.loadAvailableCustomers();
    }

    public customerCompanyTypeChanged() {
        this.data.customerCompanyType = this.firstFormGroup.get('customerCompanyType').value;
        this.loadAvailableCustomers();
    }
    ...
    public emailContentGreetingChange() {
        this.secondFormGroup.patchValue({
            emailContentGreetingName: this.secondFormGroup.get('emailContentGreeting').value.name
        });
    }

    public emailContentSubjectChange() {
        this.secondFormGroup.patchValue({
            emailContentSubject: this.secondFormGroup.get('emailContentSubject').value.name
        });
    }

    public emailContentSignatureChange() {
        this.thirdFormGroup.patchValue({
            emailContentSignatureName: this.secondFormGroup.get('emailContentSignature').value.name
        });
    }

    public generatePreview() {
        this.isLoadingPreview = true;
        this.distributionService.previewDistribution(this.data).subscribe((data: any) => {
    ...
    public distributePricingClicked() {
        this.data.emailContentGreeting = this.secondFormGroup.get('emailContentGreeting').value;
        this.data.emailContentGreeting.name = this.secondFormGroup.get('emailContentGreetingName').value;
        this.data.emailContentGreeting.subject = this.secondFormGroup.get('emailContentSubject').value;
        this.data.emailContentSignature = this.thirdFormGroup.get('emailContentSignature').value;
        this.data.emailContentSignature.name = this.thirdFormGroup.get('emailContentSignatureName').value;

        this.distributionService.distributePricing(this.data).subscribe((data: any) => {
    ...
    private loadAvailablePricingTemplates() {
        this.pricingtemplatesService.getByFbo(this.data.fboId).subscribe((data: any) => {
            ...
            if (!this.data.pricingTemplate || this.data.pricingTemplate.oid == 0)
                ...
                for (let pricingTemplate of this.availablePricingTemplates) {
                    if (this.data.pricingTemplate.oid == pricingTemplate.oid)
    ...
    private loadAvailableCustomers() {
        ...
        this.customerInfoByGroupService
            .getCustomersByGroupAndFBOAndPricing(this.data.groupId, this.data.fboId, this.data.pricingTemplate.oid).subscribe(
                (data:
                    any) => {
                    this.availableCustomers = [];
                    for (let customer of data) {
                        if (customer.distribute &&
                            (!this.data.customerCompanyType || this.data.customerCompanyType === 0 || this.data.customerCompanyType == customer.customerCompanyType))
                            this.availableCustomers.push(customer);
                    }
                    this.availableCustomers.splice(0, 0, { oid: 0, company: '--All Applicable Customers--' });
                    if (!this.data.customer || this.data.customer.oid == 0)
                        this.data.customer = this.availableCustomers[0];
                    else {
                        for (let customer of this.availableCustomers) {
    ...
    private loadAvailableCustomerCompanyTypes() {
        this.customerCompanyTypesService.getForFbo(this.data.fboId).subscribe((data: any) => {
            this.customerCompanyTypes = data;
            this.customerCompanyTypes.splice(0, 0, { oid: 0, name: '--All Types--' });
    ...
    private loadAvailableEmailContent() {
        this.emailContentService.getForFbo(this.data.fboId).subscribe((data: any) => {
            ...
            this.emailContentGreetings.splice(0,
                0,
                { oid: 0, emailContentType: 1, emailContentHtml: '', name: 'New Greeting' });
            this.emailContentSignatures.splice(0,
                0,
                { oid: 0, emailContentType: 3, emailContentHtml: '', name: 'New Signature' });
            this.data.emailContentGreeting = this.emailContentGreetings[this.emailContentGreetings.length - 1];
            this.data.emailContentSignature = this.emailContentSignatures[this.emailContentSignatures.length - 1];
            this.isLoadingEmailContent = false;
        });
    }

    private loadValidityMessage() {
        this.distributionService.getDistributionValidityForFbo(this.data.fboId).subscribe((data: any) => {
