...
namespace FBOLinx.Web.Services
{
    public class AirportFboGeofenceClustersService
    {
        ...
        public async Task<List<AirportFboGeofenceClusters>> GetAllClusters(int acukwikAirportId = 0)
        {
            ...
            var airports = await _degaContext.AcukwikAirports.Where(x => airportIds.Contains(x.Id)).Include(x => x.AcukwikFbohandlerDetailCollection)
                .ToListAsync();

            allFboGeoClusters.ForEach(x =>
            {
                var airport = airports.FirstOrDefault(a => a.Id == x.AcukwikAirportID);
                if (airport == null)
                    return;
                x.Icao = airport.Icao;
        ...
        public async Task<IQueryable<AirportFboGeofenceClusters>> GetAllClustersIQueryable(int acukwikAirportId = 0)
        {
            ...
            var airports = _degaContext.AcukwikAirports.Where(x => airportIds.Contains(x.Id)).Include(x => x.AcukwikFbohandlerDetailCollection).AsNoTracking();
