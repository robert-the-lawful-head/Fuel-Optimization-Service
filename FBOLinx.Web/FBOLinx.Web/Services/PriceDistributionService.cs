    ...
    public class PriceDistributionService : IPriceDistributionService
    {
        ...
        public async Task DistributePricing(DistributePricingRequest request, bool isPreview)
        {
            ...
            if (_IsPreview)
            {
                ...
                {
                    var customerAircrafts = await _context.Set<CustomerAircrafts>().Where(x => x.CustomerId == customer.CustomerId && x.GroupId == _DistributePricingRequest.GroupId).ToListAsync();
